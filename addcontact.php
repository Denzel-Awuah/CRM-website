...
if($_SERVER['REQUEST_METHOD'] == "POST"){
  ...
  if(empty(trim($_POST['TCRC']))){
    ...
    if(filter_var($_POST["TCRC"],FILTER_VALIDATE_INT))
      $TCRC = $_POST["TCRC"];
    else
      $TCRC_err = "Error at TCRC entry. Number expected";
  }

  if(empty(trim($_POST['ULINKS']))){
    $ULINKS_err = "No entry at ULINKS";
  } else {
    if(filter_var($_POST["ULINKS"],FILTER_VALIDATE_INT))
      $ULINKS = $_POST["ULINKS"];
    else
      $ULINKS_err = "Error at ULINKS entry. Number expected";
  }

  if(empty(trim($_POST['CLINKS']))){
    $CLINKS_err = "No entry at CLINKS";
  } else {
    if(filter_var($_POST["CLINKS"],FILTER_VALIDATE_INT))
      $CLINKS = $_POST["CLINKS"];
    else
      $CLINKS_err = "Error at CLINKS entry. Number expected";
  }

  if(empty(trim($_POST['showFellow']))){
    $showFellow_err = "No entry at showFellow";
  } else {
  if(filter_var($_POST["showFellow"],FILTER_VALIDATE_INT))
    $showFellow = $_POST["showFellow"];
  else
    $showFellow_err = "Error at showFellow entry. Number expected";
  }
  ...
  && empty ($bio_err))
  {
    ...
    if($stmt = mysqli_prepare($link,$sql)){
      ...
      mysqli_stmt_bind_param($stmt,"ssssssisiiiis", $title, $firstName, $lastName, $workEmail, $workPhone, $workLocation, $institutionID ,
